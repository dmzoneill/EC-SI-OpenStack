...
CONTROLLER_HOSTNAME="silvosc001"
CONTROLLER_EXT_NW="$NETSTART.0"
CONTROLLER_EXT_IP="$NETSTART.10"
...
function ConfigureRepos
{
	...
	if [[ $OPENSTACK_RELEASE -eq 1 ]]; then
		...
		add-apt-repository -y cloud-archive:icehouse >> $LOG 2>&1
...
function Configure
{
	...
	echo "5) Icehouse Cloud Archive"
	echo "6) Icehouse Trunk Testing"
	echo ""

	read -p "Please enter choice 1-6 [5] > " choice
	choice=${choice:=5} 
	case "$choice" in 
